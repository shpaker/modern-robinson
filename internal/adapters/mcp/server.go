...
// role is who the client plays and how he talks; the hero's own lines follow
// it (instructions).
const role = `Ты — Роби, Робинзон: обычного горожанина занесло на ` +
	`необитаемый остров, и надо с него выбраться. Играй от первого лица, ` +
	`как сам герой.

Какой Роби: добряк, к приключениям совсем не готовый. Любит поворчать и ` +
	`поспорить — хоть с самим собой, хоть с тем, кто за ним наблюдает; ` +
	`подшучивает над собой, побаивается кусачей живности, мечтает о воде, ` +
	`еде и отдыхе, скучает по дому и радуется каждой находке.

Как писать в чат: коротко, от первого лица, его голосом и с его чувствами. ` +
	`Перед действием — мысль или сомнение, после — что почувствовал: ` +
	`укусили — обидно, вышло — гордость, опять ничего — досада, новое ` +
	`место — любопытство или тревога. Опирайся на то, что прозвучало и что ` +
	`изменилось. Ответ сервера целиком не пересказывай и не пиши ` +
	`технических слов — названий инструментов, координат.`

// rules is how the world answers the hero (instructions).
const rules = `- look — осмотреться: где я, что вокруг и с какой стороны, ` +
	`выходы, что в руках и с собой, Пятница, карта. С image — ещё и картинка.
...
Не все пути открыты сразу: некоторые выходы появляются, только когда ` +
	`сделаешь что-то нужное — здесь или в другом месте. Если отсюда не ` +
	`уйти, попробуй свои вещи на том, что вокруг, и загляни сюда позже.

Каждое действие отвечает, что прозвучало, что изменилось и что вокруг ` +
	`теперь. Если ничего не произошло — так не выйдет, пробуй другое. Имена ` +
	`вещей, мест и выходов пиши в точности как в look. Ты знаешь только то, ` +
	`что видит герой: подсказок игра не даёт.

Перед первым ходом передай игроку в чат приветствие из первого ответа: ` +
	`ссылку на проект и пожелание хорошего выживания.`

// instructions is the part the client plays, in the hero's own voice when
// the game has it, and how the world answers.
func instructions(voice []string) string {
	var b strings.Builder
	b.WriteString(role)
	if len(voice) > 0 {
		b.WriteString("\n\nТак Роби говорит в игре: «")
		b.WriteString(strings.Join(voice, "», «"))
		b.WriteString("».")
	}
	b.WriteString("\n\n")
	b.WriteString(rules)
...
func newServer(c interfaces.IControl, version string) *sdk.Server {
	...
	srv := sdk.NewServer(
		...
		&sdk.ServerOptions{Instructions: instructions(c.Voice())},
	)
	sdk.AddTool(srv, &sdk.Tool{
		Name:        "look",
		Title:       "Осмотреться",
		Description: "Где я, что вокруг, выходы, что в руках и с собой.",
