...
func (g *Game) loadScene(name string, spawn *[2]int) {
	...
	g.hotspots = nil
	for _, ref := range g.sc.Objects {
		ob := g.objects[strings.ToLower(ref.Name)]
		if ob == nil {
			continue
		}
		sx, sy := g.grid.ToScreen(ref.GX, ref.GY)
		x, y, w, h := ob.ActiveZone[0], ob.ActiveZone[1], max(ob.ActiveZone[2], 8), max(ob.ActiveZone[3], 8)
		g.hotspots = append(g.hotspots, hotspot{
			key: ref.Name, ob: ob, rect: image.Rect(sx+x, sy+y, sx+x+w, sy+y+h),
		})
	}
	...
	g.audio.LoadStep(nil)
	if sv, ok := g.sc.SoundVars["step"]; ok {
		g.audio.LoadStep(g.res.Sound(sv[0]))
	}
}
...
// Draw renders the scene, character, message, and optional debug overlay.
