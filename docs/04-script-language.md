...
| `ClosedDir` | запрет **выхода** из ячейки `x,y` шагом в нумпад-направлении `d`; обе ячейки остаются проходимы, обратный шаг закрывают отдельной записью |
...
| `ClosedVert`/`ClosedDir` | стены и заборы объекта относительно его клетки (`dx,dy` / `dx,dy,d`): ставятся при появлении, снимаются при удалении |
