...
SoundVariables  step,"step.wav",1; frstep,"step_pp.wav",1; ...
                fon1,"sound11.wav",5,*; fon1,"sound12.wav",5,*; ...
TextVariables   ...
Music           ...
End;
```
...
| `ClosedDir` | запрет **выхода** из ячейки `x,y` шагом в нумпад-направлении `d` (обе ячейки остаются проходимы). Асимметричен: обратный переход закрывают отдельной записью, и авторы почти всегда пишут пары. Проверяется только поиском пути (`TryStep 0x410d00`), угловых правил нет — диагональ «сквозь» препятствие закрывают именно им (в SCENA0 это обе диагонали через угол четырёх клеток со стволом пальмы) |
| `ObjectList` | объекты на сцене: `имя, gx, gy [, *]` (координаты в сетке; `*` — особый флаг) |
| `SoundVariables` | именованные звуки: `имя,"файл.wav",голоса[,*]` (см. ниже) |
...
### SoundVariables: третье поле и звёздочка

Третье поле — **не канал**, а число одновременных голосов звукового буфера
(`0x413C92` передаёт его в создание буфера; у литеральных `Sound "file.wav"`
там жёсткая `1`). Поэтому у шагов стоит `1`, у барабанов людоедов `2`, а у
фоновых звуков 5–7: они должны накладываться друг на друга. Выбор голоса
(`0x422dd0`): текущий, если он молчит; иначе при нескольких — следующий по
кругу (если звучит, его обрывают и запускают заново); единственный занятый
голос повтор **пропускает**. Поэтому прибой SCENA0 (`wave0`, 6,8 с, скрипт
дёргает его каждые 2,3 с) звучит целиком, а не рвётся на гребне. Ремейк —
`IAudio.PlayVoice`.

Четвёртое поле `*` объявляет **фоновый пул сцены** (`fon1`…`fon9` — от «фон»).
Такие записи не запускает ни один скрипт: движок сам в своём пер-сценовом тике
(`0x414230`) выбирает случайную запись пула, играет её однократно со случайным
затуханием (до −30 дБ) и случайной панорамой (`rand()%8000-4000` при полной
шкале DirectSound ±10000), после чего ждёт `rand()%3000` мс. При входе в сцену
таймер уже истёк, так что первый звук может прозвучать сразу. Записи пула носят
одно имя и адресуются позицией, а не именем — на диске это чириканье птиц
(`bird01..07`), крики чаек (`chaiki*`), сверчки (`insect01`) и безымянные
`sound11..52`.
...
| `Cursor` | курсор над зоной: 0 — предмета в руке, 1..4 — стрелки выхода ◄ ► ▲ ▼ ([08](08-scene-objects.md#курсоры)) |
| `Text` | id всплывающей подписи (индекс в таблице текста) |
| `ClosedVert`/`ClosedDir` | стены и заборы объекта **относительно его клетки** (пары `dx,dy` / тройки `dx,dy,d`): ставятся при появлении на сцене, снимаются целиком при удалении (`Object::UnClose`) — пойманный краб и прогнанный крокодил открывают брод, на котором лежали. Носят их 50 объектов, от камней (`0,0,3; 1,1,7` — диагональ через угол) до крокодила (11 клеток) и банановых пальм (односторонний запрет сходить вниз со всего ряда над кроной) |
...
| `Aproach` | 571 | подойти к точке/объекту; кадр стоит, пока идёт владелец фильма |
...
## `GoScene` и `StartGame` останавливают кадр

Внутри одного кадра команды идут не «все подряд»:

- **`GoScene` терминальна** — скрипт на ней заканчивается, и всё, что записано
  в кадре ниже, не выполняется. Это существенно: скрипты путешествий по карте
  держат в одном кадре 3–4 `GoScene`, где условные ветки идут первыми, а
  безусловная по умолчанию — последней (`MAPSCR/ROHANBAN.FS`). Если исполнять
  весь кадр, побеждает последняя, и условные ветки становятся мёртвыми.
- **`StartGame` приостанавливает скрипт** до конца мини-игры. Ветвление по её
  результату записано в **том же кадре** (`StartGame 1,House,Find7; If House,1;
  … Endif; If House,0; … Endif` — `SCENA5/ROHANBR8.FS`), поэтому условия нельзя
  вычислять до запуска: переменная результата ещё держит старое значение, и
  ветка успеха оказывается недостижимой. Хвост кадра нужно перепроверить после
  того, как мини-игра записала результат.

Так во всех 13 скриптах-запусках мини-игр и в 438 скриптах, где условный
`GoScene` соседствует с безусловным.
