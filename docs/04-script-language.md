...
## `.SCR` — бинарный скрипт ролика (не разобран)

Внутри `.MV` есть один `.SCR` (метод `0x80`, LZHUF). Распаковывается корректно,
но это **бинарный** формат (не текст) — вероятно, скомпилированная раскадровка:
тайминги и, возможно, порядок кадров `.NGB`. Для ремейка не блокирует (кадры и
их число известны из имён `.NGB` и заголовков), но требует отдельного разбора,
если нужна точная авторская раскадровка.
