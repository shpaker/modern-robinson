...
- **Геометрия сцены.** Формула «клетка → экран» из `.SCN`
  (`LeftTopGrid`/`GridSize`/`GridShift`) не сходится: скрипт ставит героя в
  клетку, которая по ней уходит за экран. Так же не выяснено, как движок
  размещает спрайты объектов: холсты бывают `640×480` и `1024×768`, а поле
  `Shift` из заголовка `.FS` — не простое смещение. Разбирается.
- **Шашки**: превращение в дамку и алгоритм противника (`MINIGAME.DLL`,
  `0x100058F0 → 0x100065B0`).
- **Воздушный шар**: экранная проекция островов (замкнутая форма) и взлётная
  фаза.
- **`.SCR`**: смысл трёх полей заголовка (`[3]`, `[6]`, `[7]`).
- **`Delay < 0`** в кадровых скриптах и третий аргумент `Sound` (кадры
  лип-синка или позиция в ролике).
- Флаг `n` в `ObjectList` и `*` в `SoundVariables`.
