...
Источники: образ диска `ROBINSON.iso`, распакованный в папку, и руководство
игрока `DOCUMENT/ROBY.PDF` с него (44 стр.; гл. 5–6 — меню, панель, мини-игры).
...
| Мини-игры (правила: `MINIGAME.DLL` + ROBY.PDF) | 🟡 частично (ИИ шашек, проекция шара) | [10-minigames.md](10-minigames.md) |
...
опции, музыка, катсцены, оба персонажа, все шесть мини-игр. Это подтверждает
главный вывод: геймплей задан данными, и ремейк сводится к интерпретатору
ресурсов. Места, где «очевидная» реализация расходится с движком, собраны в
[06-remake-roadmap.md](06-remake-roadmap.md). Сверка с руководством игрока
(`DOCUMENT/ROBY.PDF`) выявила оставшиеся расхождения в поведении панели и
мини-игр — список в [../todo.md](../todo.md), этап 8.

## Инструментарий

Инструменты реверса — в [`../tools/`](../tools), чистый Python без сторонних
зависимостей (рядом — Go-утилиты: `questmap`, `organnotes` — высоты нот органа и
арии Пятницы, а для веб-сборки `packweb`, `webicon`, `webserve`):
...
- [11-web-build.md](11-web-build.md) — браузерная сборка: почему всё грузится сразу, хостинг, отладка.
...
- **Шашки**: алгоритм противника (`MINIGAME.DLL`, `0x100058F0 → 0x100065B0`) —
  ремейк играет упрощённо (случайный ход с обязательным взятием). Правила дамки и первого хода дало
  руководство (ROBY.PDF, стр. 32) — см. [10-minigames.md](10-minigames.md).
...
- **`.SCR`**: смысл полей заголовка помимо размера холста (`0x10/0x14`) и
  origin (`0x18/0x1C`).
- **`Delay < 0`** в кадровых скриптах: абсолютная задержка, не делящаяся на
  слайдер скорости, — зачем она авторам (см. [07-fs-scripting.md](07-fs-scripting.md)).
