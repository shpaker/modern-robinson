...
В [`../remake`](../remake) — работающий прототип на Python + pygame поверх этих
ресурсов: рендер родных сцен, персонаж Робинзон ходит по изосетке (клик →
BFS-путь), кликабельные объекты, **переходы между всеми локациями острова**
(`GoScene`). Это подтверждает главный вывод: геймплей задан данными, и ремейк
сводится к интерпретатору ресурсов. Запуск и статус — в
[`../remake/README.md`](../remake/README.md).
