...
катсцены, статичные «маски» дверей). Четыре действия — `ROHANRP1`, `ROROPBAN`
(SCENA3, конец верёвки у бананов), `ROHANBR2`, `ROHANSTI` (SCENA4, плот) — так
ждут клика игрока: `SetMouse ON; LockBar ON; AddItem X; SetActive X`, затем кадр
`Delay -N`. Клик в это время уходит в сцену с вещью `X` в руке, и запущенный им
скрипт сам снимает ожидание (`RORP1CTR`, `RORP1EMP`: `DelObject empty`,
`SetMouse OFF`); объект `empty` на весь экран с `ZCoord 1` ловит клик мимо.
Без клика кадр доигрывает, и ролик забирает вещь назад. В режиме `-mcp`
ожидание держится до хода водителя.
...
| Команда | Форма | Смысл |
|---|---|---|
| `Sound` | `ID,ch` / `"file",ch` / `"file",ch,frames` / `ID,ch,frames` | звук; имя через `SoundVariables` сцены. 3-й арг `N` звуком не управляет вовсе (исполнитель его не читает) — это **ретайминг лип-синка**: при загрузке скрипта (`RetimeByVoice`, `0x416F60`) `Delay` следующих `N` кадров, начиная с кадра самого `Sound`, перезаписывается на `⌊⌊(size−40)·1000/44100⌋/N⌋` мс — анимация подгоняется под озвучку, не наоборот (авторские Delay под репликами — заглушки, 0.17–6.2× от WAV). Знак отрицательного `Delay` сохраняется, нулевой пропускается (но слот тратит), последний `Sound` кадра выигрывает. У литеральной формы `ch` — слот, новый звук глушит предыдущий в том же слоте (резко, без fade); у именованной оригинал `ch` игнорирует и играет голосами записи (`0x41B73B`, выбор голоса — `0x422dd0`, см. `04-script-language.md`), ремейк так же (`IAudio.PlayVoice`) |
...
| `Aproach` | `char,obj,dx,dy` / `char,gx,gy` | подойти к объекту+смещение / к абсолютной клетке. Фильм стоит на кадре, пока не дойдёт его владелец; другой персонаж идёт параллельно, фильм его не ждёт. Скрытый персонаж в оригинале идёт невидимо и без звука — ремейк сразу ставит его в цель. Команда работает в любом кадре (656 скриптов держат её в кадре 1+: `SHIP2/ROHANOUT` шлёт Пятницу кадром 0 и героя кадром 2) |
...
| тумблеры | `ON`/`OFF` | `SetMouse`/`SetMap`/`LockBar`/`SetBar`/`ShowCursor`/`Interrupt`. `SetMouse OFF` глушит только сцену — клики (хит-тест, ходьбу), hover-подписи, и меняет курсор на «часы» (не прячет); панель гейтит `LockBar`, клавиатуру — никто, клик-скип катсцены работает и при OFF. Движок сам выключает мышь на любом муви и **включает по его концу** (`0x40e936`/`0x40ebea`) — 51 скрипт кончается на `SetMouse OFF` и полагается на это; после загрузки сейва мышь форсится ON (`0x4213cf`). `ShowCursor` — буквально `USER32!ShowCursor`: прячет отрисовку, ввод не трогает |
...
Frame 12,2; Delay 372; Sound "rs9011.wav",1,9;    озвучка: 9 кадров растянутся
                                                  под WAV (3302 мс → по 366)
                       Text 1000,1;               субтитр
...
- Точный смысл `Delay < 0`: плеер (`0x41661A`) берёт `|Delay|` мс, но не делит
  на масштаб слайдера скорости — «абсолютный» кадр; ретаймер знак сохраняет.
