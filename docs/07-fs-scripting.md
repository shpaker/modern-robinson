...
| Команда | Форма | Смысл |
|---|---|---|
| `Sound` | `ID,ch` / `"file",ch` / `"file",ch,frames` / `ID,ch,frames` | звук; имя через `SoundVariables` сцены; 3-й арг — кадры лип-синка озвучки (`< TotalFrames`). У литеральной формы `ch` — слот, новый звук глушит предыдущий в том же слоте; у именованной оригинал `ch` игнорирует и играет в собственном буфере записи (`0x41B73B`), ремейк пока и её ведёт по слоту |
