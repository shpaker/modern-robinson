...
| `Aproach` | `char,obj,dx,dy` / `char,gx,gy` | подойти к объекту+смещение / к абсолютной клетке. Блокирует воспроизведение: фильм стоит на кадре, пока персонаж не дойдёт, поэтому команда работает в любом кадре (656 скриптов держат её в кадре 1+: `SHIP2/ROHANOUT` шлёт Пятницу кадром 0 и героя кадром 2) |
